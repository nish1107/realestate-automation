function touchenex_nativecall(message, callback) {
    const pageurl = document.location.origin + document.location.pathname;
    let EVENT_FROM_PAGE = "__touchenex__rw_chrome_ext_" + btoa(pageurl);
    let EVENT_REPLY = "__touchenex__rw_chrome_ext_reply_" + btoa(pageurl);
        ...
        if (message.cmd == "setcallback") {
            ...
            if (transporter == undefined) {
                if(message.exfunc.fname == "new"){
                    ...
                    tag_id = "_touchenex_dummy_id_" + new Date().getTime();
                    ...
                    transporter.addEventListener(EVENT_REPLY, function (event) {
                        ...
                        if (idx > -1) {
                            ...
                            dispatchEvent(new Event("__touchenex_extension_setcallback__"));
                        }
                    });
                    (document.body || document.documentElement).appendChild(transporter);

                    window.addEventListener('__touchenex_extension_setcallback__', function(event){
            ...
            let tag_id = "_touchenex_dummy_id_" + new Date().getTime();
